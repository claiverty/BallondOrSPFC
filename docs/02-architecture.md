...
React/Vite/Router + TanStack Query para estado remoto; RHF/Zod em formulários. NestJS/Fastify com módulos de domínio; contratos Zod compartilhados. PostgreSQL no Supabase, schema `awards` isolado do legado. Supabase Auth Discord verifica token remotamente; Discord ID obtido da identidade do provedor, nunca de metadata editável. Roles privadas no banco e Guards. PostgreSQL acessado apenas pelo backend com conexão pooler e secrets. Schema não exposto no PostgREST; RLS habilitada em todas as tabelas, privilégios anon/authenticated revogados. API pública retorna apenas DTOs permitidos. Não há escrita direta pelo browser.
